...
public class EmployeServiceImplTest {
	...
	{    
	   ...
	   @Test
		...
		{
			es.affecterEmployeADepartement(1, 6);
		...
		public void affecterContratAEmployeTest()
			{
				es.affecterContratAEmploye(9,1);
				Contrat d =employeRepository.findById(1).get().getContrat(); 

				Assertions.assertThat(d.getReference()).isEqualTo(9);
			}
		...
		{
			List<Employe>  s=es.getAllEmployeByEntreprise(entrepriseRepoistory.findById(3).get());
			Assertions.assertThat(s.size()).isGreaterThan(0);
	...
	/*	@Test
		public void getSalaireMoyenByDepartementIdTest() {
			Double e= es.getSalaireMoyenByDepartementId(6);
	 ...
	 public void updateEmployeeTest(){
			...
			es.mettreAjourEmailByEmployeId("papa2@outlook.fr", 1);
	...
	{
		es.desaffecterEmployeDuDepartement(1, 6);
		List<Departement> d =employeRepository.findById(1).get().getDepartements(); 
	 ...
	 {
		    es.deleteEmployeById(25);
	        Employe employee1 = null;

	        Optional<Employe> optionalEmployee = employeRepository.findById(25);
